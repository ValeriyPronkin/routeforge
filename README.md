...
Инструмент вырос из первой из этих задач — планирования вывоза ТКО. Отсюда
отраслевые примеры в документации и заголовки вроде `tko в день (кг)` среди
распознаваемых при чтении файлов. Ядро от предметной области не зависит:
точка, база, спрос, вместимость машины — величины, а не мусорные баки.

![Пример расчёта](docs/demo.png)

*240 точек, 3 базы, машины вместимостью 4000 единиц спроса. Слева — распределение
по базам, справа — 10 маршрутов общей длиной 604 км. Воспроизводится скриптом
`scripts/make_demo_assets.py`; числа могут немного отличаться — солвер
недетерминирован.*
